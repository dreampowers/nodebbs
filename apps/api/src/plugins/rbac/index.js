...
import { setupCategoryScope } from './categoryScope.js';
 ...
 *   - categoryScope.js  分类作用域解析器接线点（setCategoryScopeResolver / resolveCategoryScope）
...
async function rbacPlugin(fastify) {
  // 1) 分类作用域解析器接线点（供 forum 模块注入 categories 父→子展开逻辑）
  setupCategoryScope(fastify);
